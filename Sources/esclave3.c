...
#define NUMERO_SERVEUR 2

char *master_ip = "127.0.0.1";

int volatile sigpipe2 = 0;

void handler_SIGPIPE3(int sig) {
    printf("Le client a fermé la connexion\n");
    sigpipe2 = 1;
}

int main(int argc, char **argv) {
    Signal(SIGPIPE, handler_SIGPIPE3);
    int clientfd;
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (server_body(connfd) == SERVER_BODY_BYE || sigpipe2) break;
        }
    }
}